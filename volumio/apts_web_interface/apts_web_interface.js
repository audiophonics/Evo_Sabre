...
function get_current_timezone(callback){
...
function server(req,res){
	...
	let postdata = ""; 
		req.on('data', (data) =>{ 
			if(data.length <500 && postdata.length <500){ // met bout à bout le stream post arrivant }	
				postdata+=(data);
			} 
			else{
				req.pause();
		...
		req.on('end', () =>{			
			let parsed = {}, DOM_response = "", parsed_string, data = postdata.toString().split('&');
			for(p of data){
				pp = p.split("=");
				parsed[pp[0]] = pp[1];
			};
			if(parsed['area']&&parsed['zone']&&options.includes(`${parsed['area']}/${parsed['zone']}`)){
				set_timezone(`${parsed['area']}/${parsed['zone']}`,(success,proc_data)=>{
					DOM_response = (success)?`Timezone updated : ${parsed['area']}/${parsed['zone']}`:proc_data;
					res.writeHead(200, {"Content-Type": "text/html"});
					res.end(make_page(DOM_response));	
				})
			}else{
				res.writeHead(200, {"Content-Type": "text/html"});
				res.end(make_page("Error : " + postdata.toString() + "is not a valid timezone"));	
			}
		})
	}
...
function make_page(DOM_response){
	DOM_response = DOM_response || "";
	let html = `
<html>
	<head></head>
	<body><div>${DOM_response}</div><form id ="tz" action="/" method="post"> <button type="submit">Send</button><br></form></body>
<script>
	current_timezone = ${JSON.stringify(current_timezone)};
	const options = ${JSON.stringify(options)};
	let list = tz_list();
	var zone_label = document.createElement("label"); zone_label.innerText = "Zone "; zone_label.for = "zone"
	var zone_selector = document.createElement("select");
	zone_selector.name = "zone";
	
	var area_label = document.createElement("label"); area_label.innerText = "Area "; area_label.for = "area"
	var area_selector = document.createElement("select");
	area_selector.name = "area";
	area_selector.onchange = pop_zone;
	document.getElementById("tz").append(area_label, area_selector, document.createElement('br'),zone_label, zone_selector);
	for(let i in list){
