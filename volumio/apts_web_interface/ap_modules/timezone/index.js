...
function set_timezone(zone,city,cb){
	cp.exec(`sudo -n sh ${__dirname}/settime.sh ${zone}  ${city}`,handle);
	function handle(e,stdout,stderr){
		let regtest = new RegExp(`${zone}/${city}`,"i");
		console.log("result : ",e,stdout,stderr)
		if(e){cb(false,e)}
		else if(stdout && regtest.test(stdout)){
			current_timezone = parse_tz(stdout);
			cb(true,stdout)
		}
		else{
			cb(false,"Error : failed to set Timezone to " + `${zone}/${city}` + "\n( "+stdout+" )")
		}
	}
}
