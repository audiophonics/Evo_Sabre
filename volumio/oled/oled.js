...
Oled.prototype._initialise = function(callback) {
	seq = [
		...
		{val : CMD_SETCLKFREQ, cmd : true}, {"dc":true}, {val : 0xF1}, {"cd":true},					// divisor  
...
function BufferBlock(col, row, buf_start){
	this.col_start = parseInt( col ) + 0x1c ;
	this.row_start = row;
...
BufferBlock.prototype.set_ending_address = function(col,row, buf_end ){
	this.col_end = parseInt( col ) + 0x1c;
	...
	//console.log(parseInt( col ) + 0x1c , 0x77, " - ", row, 64 ) 
	
	this.buf_end = buf_end+2;
	
}

BufferBlock.prototype.delimit_buffer = function(buf){
	this.buf = buf.slice(this.buf_start,this.buf_end  );
