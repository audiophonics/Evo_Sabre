...
Oled.prototype._initialise = function(callback) {
	seq = [
		...
		{val : CMD_COL }, {"dc":true}, {val : 28}, {val : 28+this.horizontal_columns -1 }, {"cd":true},	 	// column address 
		{val : CMD_ROW }, {"dc":true}, {val : 0x00}, {val : this.HEIGHT-1}, {"cd":true},	 				// row address 
		{val : CMD_SETCLKFREQ}, {"dc":true}, {val : 64}, {"cd":true},										// divisor  
		{val : CMD_MUXRATIO}, {"dc":true}, {val : this.HEIGHT-1},  {"cd":true},			 					// multiplex 
		...
		{val : CMD_MSTCONTRST},{"dc":true},  {val : this.contrast},  {"cd":true},				// master current
		{val : CMD_DEFGRYTABLE}, 
...
Oled.prototype.update = function(callback){
	...
	let sequence = [
		{val : Buffer.from([CMD_COL]) }, {"dc":true}, {val : Buffer.from([28 , 28+this.horizontal_columns-1])}, {"cd":true},	 // column address 
		{val : CMD_ROW }, {"dc":true}, {val : Buffer.from([0x00,  this.HEIGHT-1])}, {"cd":true},	 							// row address 
...
Oled.prototype.turnOffDisplay = function() {
 this.send_instruction_seq( [{val : CMD_DISPALLOFF} ] );
}
...
Oled.prototype.setContrast = function(contrast){
	if(contrast < 0 && contrast > 255) return;
	
	let seq = [
	{val: CMD_MSTCONTRST}, {"dc":true}, {val : contrast},{"cd":true}
	];
	this.send_instruction_seq(seq);
}
