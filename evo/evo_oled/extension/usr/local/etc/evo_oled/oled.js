...
var Oled = function(opts) {
  ...
  console.log("[OLED] : buffer length : ",this.bufferlength, "horizontal chunks : ", this.horizontal_chunks );
...
Oled.prototype.CacheGlyphsData = function(string){
	this.cached_glyph = {};
	if(!this.hex_font){console.log("[OLED] : font not loaded"); return}
...
Oled.prototype.load_and_display_logo = function(path, callback){
  
  if( !fs.existsSync(path) )  path = "logo.logo";
	...
	fs.readFile(path,(err,data)=>{
		try{
			...
			console.log("[OLED] : error while displaying logo", e)
			callback(false);
		}
	});
}
